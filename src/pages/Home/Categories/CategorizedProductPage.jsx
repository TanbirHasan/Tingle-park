...
const CategorizedProductPage = () => {
	...
	const { data: categorizedProducts = [], refetch, isLoading, isPreviousData } = useQuery({
		queryKey: ['categorizedProducts', location.state._id, page, size],
		queryFn: async () => {
			const res = await fetch(
				`${baseUrl}/products/category/${location.state._id}?page=${page}&size=${size}&searchProducts=${searchProducts}`
			);
